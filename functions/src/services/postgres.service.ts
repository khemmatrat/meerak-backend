...
export interface KYCSubmission {
  ...
  backgroundCheckRiskLevel: 'low' | 'medium' | 'high';
  kycStatus: string;
  kycLevel: string;
  submittedAt: Date;
  updatedAt: Date;
...
export class PostgresService {
  ...
  async saveKYCSubmission(data: Omit<KYCSubmission, 'id' | 'updatedAt'>): Promise<{ id: string }> {
    const query = `
  ...
  async getKYCStatus(firebaseUid: string): Promise<KYCSubmission | null> {
    const query = `
  ...
  async createTransaction(data: {
    ...
    try {
      ...
      const kycCheck = await client.query(
        'SELECT kyc_status FROM kyc_submissions WHERE firebase_uid = $1',
        [data.userId]
      );
  ...
  // Helper methods
  private mapRowToKYCSubmission(row: any): KYCSubmission {
    return {
      id: row.id,

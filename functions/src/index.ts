...
const postgresService = new PostgresService();  // ← เพิ่มตรงนี้
...
app.get('/api/postgres/health', authenticateToken, async (req: any, res) => {
  try {
...
app.post('/api/postgres/sync-user', authenticateToken, async (req: any, res: Response) => {
  try {
...
app.get('/api/postgres/user-data', authenticateToken, async (req: any, res: Response) => {
  try {
    const userId = req.user.uid;
...
export const submitKYC = functions.https.onCall(async (request) => {
  ...
  try {
    ...
    if (!fullName || !birthDate || !idCardNumber) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "กรุณากรอกข้อมูลพื้นฐานให้ครบ (ชื่อ, วันเกิด, เลขบัตรประชาชน)"
      );
    }
    ...
    if (idCardFront && !idCardFront.includes("mock")) {
      const result = await cloudinary.uploader.upload(idCardFront, {
        folder: `kyc/${userId}`,
    ...
    if (idCardBack && !idCardBack.includes("mock")) {
      const result = await cloudinary.uploader.upload(idCardBack, {
        folder: `kyc/${userId}`,
    ...
    if (selfiePhoto && !selfiePhoto.includes("mock")) {
      const result = await cloudinary.uploader.upload(selfiePhoto, {
        folder: `kyc/${userId}`,
    ...
    throw new functions.https.HttpsError(
      "internal",
      error.message || "การยืนยันตัวตนล้มเหลว"
    );
  }
});

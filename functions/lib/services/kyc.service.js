...
class KYCService {
    ...
    async uploadDocuments(userId, documents) {
        ...
        if (documents.idCardFront && !documents.idCardFront.includes("mock")) {
            uploadPromises.push(cloudinary_1.v2.uploader.upload(documents.idCardFront, {
                folder,
        ...
        if (documents.idCardBack && !documents.idCardBack.includes("mock")) {
            uploadPromises.push(cloudinary_1.v2.uploader.upload(documents.idCardBack, {
                folder,
        ...
        if (documents.selfiePhoto && !documents.selfiePhoto.includes("mock")) {
            uploadPromises.push(cloudinary_1.v2.uploader.upload(documents.selfiePhoto, {
                folder,
    ...
    determineKYCStatus(aiResult, backgroundCheck) {
    ...
    async updateFirestoreKYC(userId, updates) {
        await this.db.collection("users").doc(userId).update({
            ...updates,
            updated_at: new Date().toISOString(),
        });
    }
    async sendNotification(userId, status, data) {

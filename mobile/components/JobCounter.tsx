...
const JobCounter: React.FC<JobCounterProps> = ({
  ...
  // Stat cards data (with null-safety)
  const statCards = [
    {
      title: 'Total Jobs',
  ...
  const completionRate = (stats?.totalJobs ?? 0) > 0 
    ? (((stats?.completedJobs ?? 0) / (stats?.totalJobs ?? 1)) * 100).toFixed(1)
    : 0;
              ...
              <div className="border border-gray-200 rounded-xl p-4">
                ...
                <div className="space-y-3">
                  {(stats?.popularCategories ?? []).slice(0, 5).map((category, index) => {
                    const percentage = ((stats?.totalJobs ?? 0) > 0) ? (category.count / (stats?.totalJobs ?? 1)) * 100 : 0;
                    return (
              ...
              <div className="border border-gray-200 rounded-xl p-4">
                ...
                <div className="flex items-end h-40 space-x-2">
                  {(stats?.weeklyTrend ?? []).map((day, index) => {
                    ...
                    const maxCount = trend.length > 0 ? Math.max(...trend.map(d => d.count)) : 0;
        ...
        {!showCharts && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
            ...
            <div className="bg-gray-50 p-3 rounded-lg">
              ...
              <div className="font-bold">฿{Math.round((stats?.totalRevenue ?? 0) / Math.max(1, stats?.totalJobs ?? 1))}</div>
            </div>
            <div className="bg-gray-50 p-3 rounded-lg">

...
export default function QuizComponent({ quiz, onSubmit }: Props) { // <-- renamed component
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 bg-white rounded shadow">
      <h3 className="text-lg font-semibold mb-4">{quiz.title ?? 'แบบทดสอบ'}</h3>
      {questions.length === 0 ? (
        <p className="text-amber-700 bg-amber-50 p-4 rounded-lg">ยังไม่มีคำถามในแบบทดสอบนี้ กรุณาติดต่อผู้ดูแลระบบ</p>
      ) : (

...
/**
 * PDPA-compliant mask for Thai National ID in summaries/dashboards.
 * Format: 1-xxxx-xxxx-xx-x (first and last digit visible only).
 * Full value should only be visible to Authorized Admins or via secure 'View' with re-authentication.
 */
export function maskID(idString: string | null | undefined): string {
  if (!idString || typeof idString !== 'string') return '—';
  const cleaned = idString.replace(/[\s-]/g, '');
  if (cleaned.length !== 13 || !/^\d{13}$/.test(cleaned)) return '—';
  return `${cleaned[0]}-xxxx-xxxx-xx-${cleaned[12]}`;
}

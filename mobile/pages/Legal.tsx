...
import { fetchComplianceTypes, fetchCompliancePolicy } from '../services/compliancePolicyService';
...
export const Legal: React.FC = () => {
  ...
  const [types, setTypes] = useState<{ type: string; version: string; published_at: string }[]>([]);
  const [activeType, setActiveType] = useState<string>('terms');
  const [policy, setPolicy] = useState<{ content: string; version: string; published_at: string } | null>(null);
  ...
  useEffect(() => {
    ...
    loadTypes();
  }, [bootstrap.complianceVersions.terms, bootstrap.complianceVersions.privacy]);
  ...
  useEffect(() => {
    ...
    const loadPolicy = async () => {
      ...
      try {
        ...
        setPolicy(
          p
            ? { content: p.content, version: p.version, published_at: p.published_at }
            : null,
        );
      } catch {
        setPolicy(null);
  ...
  const displayTypes = Object.keys(POLICY_LABELS).map((t) => typeMap.get(t) || { type: t, version: '', published_at: '' });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Legal Documents (Versioning)</h2>
          ...
          {displayTypes.map((t) => (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${
                activeType === t.type ? 'bg-slate-800 text-white shadow' : 'text-gray-500 hover:bg-gray-50'
              }`}
            >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 prose prose-slate max-w-none min-h-[200px]">
          {loadingContent ? (
            ...
            <div>
              <h2 className="flex items-center text-2xl font-bold text-slate-900 mb-6">
                <FileText className="mr-3 text-emerald-600" /> {POLICY_LABELS[activeType] || activeType}
              </h2>
              <p className="text-sm text-gray-500 mb-4">
                Version {policy.version} • {policy.published_at ? new Date(policy.published_at).toLocaleDateString() : ''}
              </p>
              <div
            ...
            <div className="text-gray-500 py-8 text-center">
              {loading ? 'กำลังโหลด...' : (
                <>
                  ...
                  <p className="text-xs mt-2 text-gray-400">เอกสารนี้อาจยังไม่ได้เพิ่มในระบบ กรุณารีเฟรชหรือติดต่อผู้ดูแล</p>
                </>
              )}
            </div>
          )}

...
import { gradeService, GradeData, GRADE_META, WorkerGrade } from "../services/gradeService";

export type ExpertCategory = "all" | "chef" | "tailor" | "artist" | "barber" | "wellness" | "driver" | "cleaning" | "technical" | "party_guest";
type GradeFilter = "all" | "A" | "B" | "C";

const EXPERT_FILTERS: { id: ExpertCategory; label: string; icon: React.ReactNode }[] = [
  { id: "all", label: "ทั้งหมด", icon: <Sparkles size={18} className="text-emerald-600" /> },
  { id: "driver", label: "คนขับรถ & แมสเซนเจอร์", icon: <Car size={18} className="text-slate-600" /> },
  { id: "cleaning", label: "ทำความสะอาด", icon: <Sparkles size={18} className="text-slate-600" /> },
  { id: "technical", label: "ช่างเทคนิค", icon: <Wrench size={18} className="text-slate-600" /> },
  { id: "party_guest", label: "เพื่อนเที่ยว / ปาร์ตี้", icon: <PartyPopper size={18} className="text-slate-600" /> },
  { id: "chef", label: "Gourmet & Chef", icon: <UtensilsCrossed size={18} className="text-slate-600" /> },
  { id: "tailor", label: "Style Masters", icon: <Scissors size={18} className="text-slate-600" /> },
  { id: "artist", label: "Entertainment", icon: <Palette size={18} className="text-slate-600" /> },
  { id: "barber", label: "Barber", icon: <Scissors size={18} className="text-slate-600" /> },
  { id: "wellness", label: "Wellness & Spa", icon: <Heart size={18} className="text-slate-600" /> },
];
...
const TalentCover: React.FC<{ src?: string | null; alt: string }> = ({ src, alt }) => {
  ...
  if (!valid || failed) {
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-slate-100 via-white to-emerald-50/60">
        <User className="text-slate-200" size={68} strokeWidth={1.15} aria-hidden />
      </div>
    );
  }
  return (
...
export const Talents: React.FC = () => {
  ...
  const HOME_CATEGORIES = ["driver", "cleaning", "technical", "party_guest"] as const;

  useEffect(() => {
    const cat = searchParams.get("category");
    const status = searchParams.get("status");
    if (cat && ["all", "driver", "cleaning", "technical", "party_guest", "chef", "tailor", "artist", "barber", "wellness"].includes(cat)) {
  ...
  const categoryToSkills: Record<string, string[]> = {
    ...
    technical: ["Plumbing", "Electrician", "Repair", "plumbing", "electrician", "ช่าง"],
    party_guest: ["Party_Guest", "Dating", "Sommelier", "party_guest", "dating", "sommelier", "เพื่อนเที่ยว"],
  };
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      setLoading(true);
      try {
        const categoryParam = expertCategory === "all" || HOME_CATEGORIES.includes(expertCategory as any)
          ? undefined
          : expertCategory;
  ...
  const toggleSaveTalent = async (e: React.MouseEvent, talentId: string) => {
    ...
    try {
      if (isSaved) {
        ...
        setSavedIds((prev) => { const s = new Set(prev); s.delete(talentId); return s; });
      } else {
  ...
  useEffect(() => {
    ...
    if (HOME_CATEGORIES.includes(expertCategory as any) && expertCategory !== "all") {
      const skills = categoryToSkills[expertCategory] || [];
      result = result.filter((p) => {
        const providerSkills = (p.skills || []).map((s) => (typeof s === "object" ? (s as any)?.name : String(s)));
        const expertCat = (p as any).expert_category;
        return skills.some((sk) => providerSkills.some((ps) => String(ps).toLowerCase().includes(sk.toLowerCase()))) ||
          (expertCat && skills.some((sk) => String(expertCat).toLowerCase().includes(sk.toLowerCase())));
      });
    }
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (p) =>
          ...
          (p.signature_service && p.signature_service.toLowerCase().includes(q)) ||
    ...
    if (savedOnly) {
      result = result.filter((p) => savedIds.has(String(p.id)));
    }

    if (onlineOnly) {
      result = result.filter((p) => (p as any).status === "available");
    }

    // Sort: Grade A first, then by rating
    result = [...result].sort((a, b) => {
      const ga = gradeMap[a.id]?.grade || "C";
      const gb = gradeMap[b.id]?.grade || "C";
    ...
    setFiltered(result);
  }, [searchQuery, genderFilter, gradeFilter, savedOnly, savedIds, providers, gradeMap, expertCategory, onlineOnly]);
  ...
  return (
    <div className="space-y-5 bg-white">
      <div>
        ...
        <p className="text-slate-500 mt-2 text-sm leading-relaxed">{t("talents.subtitle")}</p>
      </div>

      <div className="rounded-2xl border border-slate-200/90 bg-white p-4 sm:p-5 shadow-sm space-y-4">
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">{t("talents.category_label")}</p>
        ...
        <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-slate-100">
          {onlineOnly && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium bg-emerald-50 text-emerald-800 border border-emerald-200">
              ...
              Online เท่านั้น
              <button type="button" onClick={() => setOnlineOnly(false)} className="text-emerald-600 hover:text-emerald-800 font-bold">
                ×
              </button>
            </span>
          )}
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium border transition-all ${
              savedOnly ? "bg-rose-50 text-rose-800 border-rose-200" : "bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100"
            }`}
          >
            <Heart size={14} className={savedOnly ? "fill-rose-500 text-rose-500" : ""} />
            {savedOnly ? `ที่บันทึก (${filtered.length})` : "แสดงเฉพาะที่บันทึก"}
          </button>
        </div>

        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Grade</p>
          <div className="flex items-center gap-2 flex-wrap">
            {(["all", "A", "B", "C"] as GradeFilter[]).map((g) => {
              ...
              return (
                ...
                >
                  {g === "A" && <Crown size={13} className="text-amber-700" fill="currentColor" />}
                  {g === "B" && <ShieldCheck size={13} />}
                  {g === "all" ? "ทั้งหมด" : `Grade ${g}`}
                  {g === "A" && <span className="text-[10px] opacity-90">VVIP</span>}
                </button>
              );
            })}
          </div>
        ...
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">{t("talents.gender_label")}</p>
          <div className="flex flex-wrap gap-2">
            {(
              [
        ...
        <div className="relative pt-1 border-t border-slate-100">
          <Search className="absolute left-3 top-[calc(50%+2px)] -translate-y-1/2 text-slate-400 pointer-events-none" size={20} />
          <input
            type="search"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filtered.length === 0 ? (
            <div className="col-span-full text-center py-12 text-gray-500">{t("talents.no_results")}</div>
          ) : (
            filtered.map((person) => {
              const avatarUrl = person.avatar_url || (person as any).avatarUrl;
              const portfolioUrls = person.portfolio_urls || (person as any).portfolio_urls || [];
              const rawCover = portfolioUrls[0] || avatarUrl;
              const coverImage = rawCover && String(rawCover).trim() ? rawCover : null;
              const completedJobs = person.completed_jobs_count ?? (person as any).completedJobs ?? 0;
              const rating = person.rating ?? 0;
              const badge = person.verified_badge || (person as any).verified_badge;
              const greetingVideo = person.greeting_video_url || (person as any).greeting_video_url;
              ...
              return (
                <div
                  ...
                  onMouseEnter={() => greetingVideo && videoRefs.current[person.id]?.play().catch(() => {})}
                  onMouseLeave={() => greetingVideo && (videoRefs.current[person.id]?.pause(), (videoRefs.current[person.id] && (videoRefs.current[person.id].currentTime = 0)))}
                >
                  ...
                  <div className="h-64 bg-slate-100 relative overflow-hidden">
                    <TalentCover src={coverImage} alt={person.name || "Talent"} />
                    {greetingVideo && (
                      <video
                    ...
                    {person.is_online && (
                      <div className="absolute top-4 left-4 w-3 h-3 bg-green-500 border-2 border-white rounded-full shadow-sm z-10" title="Online" />
                    )}
                    <button
                      ...
                      title={savedIds.has(person.id) ? "ยกเลิกบันทึก" : "บันทึก Talent ไว้จ้างภายหลัง"}
                    >
                      <Heart
                        size={20}
                        className={savedIds.has(person.id) ? "text-rose-500 fill-rose-500" : "text-gray-500"}
                      />
                    </button>
                      ...
                      <div className="flex justify-between items-end text-white">
                        <div>
                          <h3 className="text-xl font-bold flex items-center">
                            {person.name}
                            {person.age != null && <span className="ml-2 text-sm font-normal opacity-90">{person.age}</span>}
                          </h3>
                          {person.university && (
                        ...
                        <div className="flex items-center bg-black/40 px-2 py-1 rounded-lg backdrop-blur-sm gap-1">
                          ...
                          <span className="font-bold text-sm">
                            {personGrade ? Number(personGrade.avg_rating).toFixed(1) : Number(rating).toFixed(1)}
                          </span>
                          {personGrade && (
                            <span
                              className="text-[10px] font-bold px-1 rounded"
                              style={{
                                background: GRADE_META[personGrade.grade].bgColor,
                                color: personGrade.grade === "C" ? "#fff" : "#fff",
                              }}
                            >
                              {personGrade.grade}
                            </span>
                  ...
                  <div className="p-4">
                    ...
                    {personGrade?.grade === "A" && (
                      <div
                        ...
                        style={{ background: "linear-gradient(90deg,#D4AF37,#F5E27D,#B8860B)", color: "#fff" }}
                      >
                    ...
                    {personGrade?.grade === "B" && (
                      <div className="flex items-center gap-1.5 mb-2 text-indigo-600">
                        <ShieldCheck size={13} />
                        <span className="text-xs font-bold">Professional • {personGrade.total_reviews} reviews</span>
                      </div>
                    )}
                    {!personGrade && badge && (
                    ...
                    {person.signature_service && (
                      <p className="text-sm text-gray-600 mb-2 line-clamp-2">{person.signature_service}</p>
                    )}
                    <div className="flex flex-wrap gap-1 mb-3">
                      {(person.skills || person.looks || []).slice(0, 4).map((tag: string) => (
                    ...
                    <div className="flex justify-between text-xs text-gray-500 mb-4">
                      {person.height != null && (
                        <span className="flex items-center" title={t("talents.height")}>
                          <User size={12} className="mr-1" /> {person.height} cm
                        </span>
                      )}
                      <span className="flex items-center">
                        <Briefcase size={12} className="mr-1" /> {personGrade?.total_jobs ?? completedJobs} jobs
                      </span>
                    </div>
                    <div className="flex flex-col gap-2">
                      <div className="flex gap-2">
                        <Link
                          to={person.id ? `/talents/${String(person.id)}` : "/talents"}
                      ...
                      >
                        <Heart size={16} className={savedIds.has(person.id) ? "fill-rose-500" : ""} />
                        {savedIds.has(person.id) ? "บันทึกแล้ว" : "บันทึก Talent ไว้จ้างภายหลัง"}
                      </button>
                    </div>
                  </div>
                </div>

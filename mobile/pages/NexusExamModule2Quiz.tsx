...
const PROGRESS_KEY = (userId: string, cat: string) => `nexus_m2_progress:${userId}:${cat}`;
const PROGRESS_TTL_MS = 24 * 60 * 60 * 1000; // 24 ชม.
...
export default function NexusExamModule2Quiz() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // State Persistence: โหลดคำตอบและเวลาเดิมถ้ามี (Refresh/เน็ตหลุด)
        if (user?.id) {
          try {
            const key = PROGRESS_KEY(user.id, decodeURIComponent(category));
  ...
  // บันทึกคำตอบลง localStorage ทุกครั้งที่เปลี่ยน (debounce)
  useEffect(() => {
    if (!user?.id || !category || Object.keys(answers).length === 0) return;
    if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current);
    saveTimeoutRef.current = setTimeout(() => {
      try {
        const key = PROGRESS_KEY(user.id, decodeURIComponent(category));
        localStorage.setItem(key, JSON.stringify({ answers, startedAt }));
      } catch (_) {}
      saveTimeoutRef.current = null;
    }, 500);
    return () => { if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current); };
  }, [answers, startedAt, user?.id, category]);
  ...
  const clearProgress = () => {
    if (user?.id && category) {
      try {
        localStorage.removeItem(PROGRESS_KEY(user.id, decodeURIComponent(category)));
      } catch (_) {}
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.passed) {
        // clear profile cache เพื่อให้ Profile.tsx โหลดข้อมูลใหม่แทนที่จะเจอ cached version
        try {
          const uid = user.id;
          const key = `profile:${uid}`;
          // ลบ cache จาก in-memory map ผ่าน localStorage flag
          localStorage.setItem('meerak_cert_cache_bust', Date.now().toString());
        } catch (_) {}
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6">
        ...
        <button onClick={() => navigate('/training/nexus-module2')} className="text-indigo-600">
          ← {t('training.select_new_category')}
        </button>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 to-white p-6">
        <div className={`rounded-2xl p-8 max-w-md w-full text-center ${result.passed ? 'bg-green-50 border-2 border-green-200' : 'bg-amber-50 border-2 border-amber-200'}`}>
          <p className="text-2xl font-bold text-slate-800">
            {result.passed ? t('training.passed_m2') : t('training.failed')}
          </p>
    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-10">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4">
          ...
          <button
            onClick={() => { clearProgress(); navigate('/training/nexus-module2'); }}
            className="text-slate-500 hover:text-slate-700 text-sm"
          >
            {t('training.cancel')} — {t('training.select_new_category')}
          </button>
        </div>

...
import { TransportSkillGuide } from "../components/TransportSkillGuide";
...
function aqondTrainingCopy(text: string, lang: Language): string {
  ...
  if (lang === "th") {
    return text
      .replace(
        /มาตรฐานการบริการและความปลอดภัยของ Nexus/g,
        "มาตรฐานการบริการและความปลอดภัยของ AQOND",
      )
      .replace(/ลำดับขั้นตอน Nexus Exam/g, "ขั้นตอนการอบรมและสอบมาตรฐาน AQOND")
      .replace(
        /Nexus Exam — Module 2 & 3/g,
        "การสอบมาตรฐาน AQOND — โมดูล 2 และ 3",
      )
  ...
  return text
    .replace(
      /"Nexus Service Standards & Safety"/g,
      '"AQOND Service Standards & Safety"',
    )
    .replace(/Nexus Exam Steps/g, "AQOND training & exam steps")
    .replace(
      /Nexus Exam — Module 2 & 3/g,
      "AQOND certification — Modules 2 & 3",
    )
    .replace(/\bNexus\b/g, "AQOND")
    .replace(/\bNEXUS\b/g, "AQOND");
}
...
export default function TrainingDashboard() {
  ...
  const tr = useCallback(
    (key: string) => aqondTrainingCopy(t(key), language),
    [t, language],
  );
  ...
  const [tab, setTab] = useState<
    "overview" | "courses" | "progress" | "certificates"
  >("overview");
  const [providerStatus, setProviderStatus] = useState<string | null>(null);
  const [onboardingStatus, setOnboardingStatus] = useState<string | null>(null);
  const [kycStatus, setKycStatus] = useState<string | null>(null);
  const [examResults, setExamResults] = useState<
    { module: number; passed: boolean }[]
  >([]);
  ...
  useEffect(() => {
    const state = location.state as {
      module1JustPassed?: boolean;
      examResults?: { module: number; passed: boolean }[];
      onboardingStatus?: string;
    } | null;
    if (
      state?.module1JustPassed &&
      Array.isArray(state.examResults) &&
      state.examResults.length > 0
    ) {
      setExamResults(state.examResults);
      try {
        localStorage.setItem(
          AQOND_TRAINING_EXAM_RESULTS_KEY,
          JSON.stringify(state.examResults),
        );
      } catch (_) {}
  ...
  const avgScore =
    progress.length > 0
      ? Math.round(
          progress.reduce((s, p) => s + (p.bestScore ?? 0), 0) /
            progress.length,
        )
      : 0;
  ...
  const getCourseProgress = (courseId: string): Progress | undefined => {
    ...
    if (courseId === NEXUS_PROFESSIONAL_COURSE_ID) {
      const passedFromBackend = examResults.some(
        (r) => r.module === 1 && r.passed,
      );
      ...
      if (passedFromBackend || passedFromProgress) {
        ...
        const score =
          p?.bestScore != null && p.bestScore > 0 ? p.bestScore : 85;
        return {
          ...base,
          completed: true,
          bestScore: Math.max(score, 85),
          watched: true,
        };
      }
    }
  ...
  const isProvider =
    user?.role === UserRole.PROVIDER ||
    (user?.role as string)?.toLowerCase() === "provider";
    ...
    <div className={trainingTierClass}>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6 md:p-10 training-inner">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h1 className="text-4xl font-bold text-gray-900">
                  {tr("training.dashboard_title")}
                </h1>
                <p className="text-gray-600 text-sm mt-1">
                  {tr("training.dashboard_subtitle")}
                </p>
              </div>
            </div>
          </div>
            ...
            providerStatus !== "VERIFIED_PROVIDER" && (
              <div className="mb-8 p-6 bg-amber-50 border-2 border-amber-400 rounded-xl flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-amber-800 text-sm mt-1">
                      ...
                      !examResults.some((r) => r.module === 1 && r.passed)
                ...
                <button
                  onClick={() => {
                    setTab("courses");
                    if (examResults.some((r) => r.module === 1 && r.passed))
                      navigate("/training/nexus-module2");
                    else
                      navigate("/training/course/nexus-professional-standards");
                  }}
                  className="px-6 py-3 bg-amber-600 text-white font-semibold rounded-lg hover:bg-amber-700 shadow"
                >
                  {examResults.some((r) => r.module === 1 && r.passed)
                    ? tr("training.go_to_module2")
                    : tr("training.go_to_exam")}
                </button>
              </div>
            )}
          ...
          {user?.role === UserRole.PROVIDER &&
            providerStatus !== "VERIFIED_PROVIDER" &&
            (onboardingStatus === "TRAINING_COMPLETE" ||
            ...
            !["verified", "approved"].includes(
              ...
              <div className="mb-6 p-5 bg-blue-50 border-2 border-blue-300 rounded-xl">
                <p className="font-bold text-blue-900">
                  {tr("training.training_done_kyc")}
                </p>
                <p className="text-blue-800 text-sm mt-1">
                  {tr("training.training_done_kyc_desc")}
                </p>
                <button
                  type="button"
            ...
            !examResults.some((r) => r.module === 2 && r.passed) && (
              <div className="mb-6 p-5 bg-emerald-50 border-2 border-emerald-400 rounded-xl">
                <p className="font-semibold text-emerald-900 mb-2">
                  ✅ {tr("training.passed_m1_do_m2")}
                </p>
                <p className="text-emerald-800 text-sm mb-3">
                  {tr("training.m2_instruction")}
                </p>
                <button
                  type="button"
          ...
          <section className="mb-8 grid gap-4 md:grid-cols-2">
            <button
              ...
              className="text-left rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-5 hover:border-emerald-400 transition-colors"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900">ซื้อคอร์สเพิ่มทักษะ</h3>
                  <p className="text-sm text-slate-600">
                    Marketplace แบบ Udemy สำหรับคนทำงานบริการ จ่ายด้วย Wallet และเรียนต่อในแอป
                  </p>
                </div>
              </div>
            </button>
            <button
              ...
              className="text-left rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white p-5 hover:border-indigo-400 transition-colors"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900">ลงขายคอร์สของคุณ</h3>
                  <p className="text-sm text-slate-600">
                    สร้าง draft, ใส่บทเรียน preview, ส่งให้ admin review แล้วขายบน AQOND
                  </p>
                </div>
              </div>
            </button>
          ...
          <div className="flex flex-wrap gap-2 mb-8 border-b border-gray-200 pb-4">
            {[
              ...
              {
                id: "certificates",
                label: tr("training.certificates"),
                icon: Trophy,
              },
            ].map((tabItem) => {
            ...
            <div>
              ...
              {(isProvider || examResults.length > 0) && (
                <div className="mb-8 p-6 bg-slate-50 rounded-2xl border-2 border-slate-200">
                  <h3 className="text-xl font-bold text-slate-800 mb-4">
                    {tr("training.nexus_m2_m3")}
                  </h3>
                  <p className="text-slate-600 text-sm mb-4">
                    {tr("training.after_m1_tap")}
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div
                      ...
                      onClick={() => navigate("/training/nexus-module2")}
                      onKeyDown={(e) =>
                        e.key === "Enter" && navigate("/training/nexus-module2")
                      }
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {examResults.some((r) => r.module === 2 && r.passed) ? (
                          ...
                          <span className="text-emerald-700 text-sm">
                            {tr("training.technical_skills")}
                          </span>
                        )}
                      </div>
                      <p className="text-emerald-800 text-sm">
                        {tr("training.select_category_exam")}
                      </p>
                    </div>
                    <div
                      role="button"
                      ...
                      onClick={() => navigate("/training/nexus-module3")}
                      onKeyDown={(e) =>
                        e.key === "Enter" && navigate("/training/nexus-module3")
                      }
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {examResults.some((r) => r.module === 3) ? (
                          ...
                          <span className="text-slate-600 text-sm">
                            {tr("training.mindset_scenario")}
                          </span>
                        )}
                      </div>
                      <p className="text-slate-700 text-sm">
                        {tr("training.scenario_exam")}
                      </p>
                    </div>
                  </div>
                  <div className="mt-4">
                    <TransportSkillGuide defaultOpen={false} compact />
                  </div>
                </div>
              )}

              <h3 className="text-2xl font-bold mb-6 text-gray-900">
                {tr("training.available_courses")}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {courses.map((course) => {
                  ...
                  const hasQuiz =
                    course.lessons?.[0]?.quiz?.questions?.length > 0;
                      ...
                      <div className="p-6">
                        ...
                        <div className="space-y-2 mb-4 text-sm text-gray-600">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              {hasQuiz
                                ? tr("training.video_quiz").replace(
                                    "{n}",
                                    String(
                                      course.lessons?.[0]?.quiz?.questions
                                        ?.length || 0,
                                    ),
                                  )
                                : tr("training.video_only")}
                            </span>
                          </div>
                        </div>
                        ...
                        >
                          {isCompleted
                            ? `✓ ${tr("training.view_certificate")}`
                            : prog?.watched
                              ? `▶ ${tr("training.continue_course")}`
                              : `▶ ${tr("training.start_course")}`}
                        </button>
                      </div>
                    </div>
                  );
                ...
                {stats.length > 0 ? (
                  stats.map((stat) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between text-sm text-gray-600">
                        ...
                        <span>
                          Best Score: <strong>{stat.bestScore}%</strong>
                        </span>
                      </div>
                    </div>
                  ))
                  ...
                  <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
                    <p className="text-gray-500">
                      No progress yet. Start a course to see your progress here.
                    </p>
                  </div>
                )}
              </div>

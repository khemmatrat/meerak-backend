...
/**
 * นโยบายความเป็นส่วนตัว — เนื้อหาจาก GET /api/compliance/privacy
 * รีเฟรชเมื่อเวอร์ชันใน bootstrap เปลี่ยน
 */
export const PrivacyPage: React.FC = () => {
  const { bootstrap } = useMobileAppConfig();
  const privacyVersionKey = bootstrap.complianceVersions.privacy ?? "";
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const p = await fetchCompliancePolicy("privacy");
        if (cancelled) return;
        if (p?.content) {
          setHtml(p.content);
    ...
    <LegalPublicShell
      ...
      subtitle={
        version
          ...
          : "การเก็บรักษาและใช้ข้อมูลส่วนบุคคล — AQOND"
      }
    >
      {loading ? (
        ...
        <p className="not-prose text-center text-slate-600 py-8">
          ยังไม่มีนโยบายในระบบ — ดูรายการเอกสารได้ที่{" "}
          <Link to="/legal?type=privacy" className="text-emerald-700 font-medium underline">
            Legal &amp; Compliance
          </Link>
        </p>
      )}
      ...
      <p className="not-prose text-sm text-slate-600 mt-6">
        กลับไปอ่านข้อกำหนดการให้บริการได้ที่{" "}
        <Link to="/terms" className="text-emerald-700 font-medium hover:underline">
          Terms of Service
        </Link>
      </p>
    </LegalPublicShell>

...
const FirebaseApi = {
  ...
  /** Provider: mark arrived — update job doc in Firestore (status + timestamps). */
  async confirmArrival(jobId: string, userId: string): Promise<void> {
    void userId;
    if (!db) {
      throw new Error(
        "Firebase ยังไม่ได้ตั้งค่า กรุณาใส่ VITE_FIREBASE_* หรือ window.__firebase_config / localStorage meerak_firebase_config"
      );
    }
  ...
  /** Fetch Verified Work Clips for a provider from Firestore (providers/{id}/verified_work_clips) */
  async getProviderWorkClips(providerId: string): Promise<{ id: string; url: string; type?: string }[]> {
    if (!db) return [];
    try {
      const clipsRef = collection(db, "providers", providerId, "verified_work_clips");
      const snap = await getDocs(clipsRef);
      return snap.docs
        .map((d) => ({ id: d.id, url: (d.data() as any).url || "", type: (d.data() as any).type }))
        .filter((c) => c.url);
    } catch {
      return [];
    }
  ...
  /** Upload job proof photo (before/after) to Firebase Storage, returns download URL */
  async uploadJobProof(jobId: string, file: File, type: "before" | "after"): Promise<string> {
    if (!app || !firebaseConfig?.storageBucket) {
      throw new Error("Firebase Storage ไม่พร้อม กรุณาตรวจสอบการตั้งค่า");
    }
    const storage = getStorage(app);
    const ext = file.name.split(".").pop() || "jpg";
    const path = `job_proofs/${jobId}/${type}_${Date.now()}.${ext}`;

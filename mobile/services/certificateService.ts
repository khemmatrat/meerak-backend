...
function generateMarketplaceCertificateHTML(
  learnerName: string,
  courseName: string,
  verifyCode: string,
  issuedDate: string,
  ...
  return `
    <div id="certificate-template" style="
      ...
      align-items: center;
    ">
      <div style="border: 3px solid #6ee7b7; padding: 30px; border-radius: 10px; max-width: 700px;">
        <p style="margin: 0 0 8px 0; font-size: 14px; letter-spacing: 2px;">AQOND COURSE MARKETPLACE</p>
        <h1 style="margin: 0 0 20px 0; font-size: 42px;">Certificate of Completion</h1>
        ...
        <p style="margin: 20px 0; font-size: 16px;">has successfully completed</p>
        <h3 style="margin: 20px 0; font-size: 28px; font-weight: bold;">
          ${courseName}
        </h3>
        <p style="margin: 20px 0; font-size: 14px;">Verify code: <strong>${verifyCode}</strong></p>
...
export const certificateService = {
  async generateMarketplaceCertificatePDF(opts: {
    learnerName: string;
    courseName: string;
    verifyCode: string;
    issuedAt: string;
    ...
    container.innerHTML = generateMarketplaceCertificateHTML(
      opts.learnerName,
      opts.courseName,
      opts.verifyCode,
      opts.issuedAt,
    ...
    try {
      ...
      const filename = `AQOND_Certificate_${opts.verifyCode}.pdf`;

 ...
 * OTP Request Types
 */
export type OTPType = 'login' | 'register' | 'verify' | 'reset_password' | 'change_phone';
...
async function checkRateLimit(
  ...
  const now = Date.now();
  const windowStart = new Date(now - RATE_LIMITS.per_phone.window).toISOString();
  ...
  if (recentPhoneOTPs.length >= RATE_LIMITS.per_phone.max) {
    return { exceeded: true, reason: `Rate limit: ${RATE_LIMITS.per_phone.max} OTPs per hour for this phone` };
  }
  ...
  if (ip) {
    ...
    if (recentIPOTPs.length >= RATE_LIMITS.per_ip.max) {
      return { exceeded: true, reason: `Rate limit: ${RATE_LIMITS.per_ip.max} OTPs per hour for this IP` };
    }
  }
  ...
  if (recentDeviceOTPs.length >= RATE_LIMITS.per_device.max) {
    return { exceeded: true, reason: `Rate limit: ${RATE_LIMITS.per_device.max} OTPs per hour for this device` };
  }
...
export async function requestOTP(
  ...
  try {
    ...
    if (rateLimitCheck.exceeded) {
      logger?.warn('Rate limit exceeded', { phone, reason: rateLimitCheck.reason });
      return {
        success: false,
...
export async function verifyOTP(
  ...
  try {
    ...
    if (!isValid) {
      logger?.warn('Invalid OTP code', { otpId, attempts: otpRecord.attempts + 1 });
      return {
        success: false,
        error: `Invalid OTP code (${otpRecord.max_attempts - otpRecord.attempts - 1} attempts left)`
      };
    }
    ...
    logger?.info('OTP verified successfully', { otpId, phone: otpRecord.phone });

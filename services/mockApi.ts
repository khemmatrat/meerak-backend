...
export const MOCK_COURSES: TrainingModule[] = [
  ...
  // คอร์สบังคับสำหรับ Provider หลัง KYC ผ่าน: มาตรฐานการบริการและความปลอดภัยของ Nexus (55 ข้อ, ผ่าน ≥85%, ไม่ผ่านรอ 24 ชม.)
  {
    id: "nexus-professional-standards",
    title: "มาตรฐานการบริการและความปลอดภัยของ Nexus",
    name: "มาตรฐานการบริการและความปลอดภัยของ Nexus",
    description: "เรียนรู้มาตรฐานการให้บริการและความปลอดภัยที่ Nexus กำหนดให้ Provider ทุกคนต้องผ่านก่อนรับงาน",
    ...
    lessons: [{ id: "l1", title: "มาตรฐานการบริการและความปลอดภัย", duration: "15 นาที" }],
    quiz: (() => {
      const opts = [
        { id: "a", text: "การปฏิบัติตามมาตรฐานความปลอดภัยและสิทธิส่วนบุคคล", isCorrect: false },
        { id: "b", text: "การให้บริการด้วยความสุภาพ ตรงเวลา และรายงานเหตุการณ์ไม่ปกติ", isCorrect: false },
        { id: "c", text: "การไม่เปิดเผยข้อมูลลูกค้าและปฏิบัติตามกฎหมาย", isCorrect: false },
        { id: "d", text: "ถูกทุกข้อ", isCorrect: false },
      ];
      const questions = [];
      for (let i = 1; i <= 55; i++) {
        const correctIndex = i % 4;
        const options = opts.map((o, idx) => ({ ...o, isCorrect: idx === correctIndex }));
        questions.push({
          id: `nexus-q${i}`,
          text: `ข้อ ${i}. มาตรฐานการบริการและความปลอดภัยของ Nexus ข้อใดถูกต้อง?`,
          type: "mcq" as const,
          options,
        });
      }
...
export const MockApi = {
  ...
  requestPasswordReset: async (phone: string): Promise<void> => {
    try {
      ...
      throw new Error(e.response?.data?.error || e.message || "ส่งคำขอไม่สำเร็จ");
    }
  },
  ...
  login: async (
    ...
    try {
      ...
      try {
        ...
        // Rate limit (429) — do not fallback; throw with retry_after for Smart Retry UI
        if (backendError?.response?.status === 429) {
          ...
          const retry = typeof data.retry_after === "number" ? data.retry_after : undefined;
          const err = new Error(retry ? `${msg} Try again in ${retry} seconds.` : msg) as Error & { retry_after?: number };
      ...
      // Fallback to Firebase (requires Firestore)
      if (!db) {
        throw new Error(
          "Login failed. Backend unavailable and Firebase is not configured. Please set VITE_FIREBASE_* env or use backend."
        );
      }
  ...
  getJobs: async (category?: string, searchQuery?: string): Promise<Job[]> => {
    try {
      try {
        ...
        console.log("[getJobs] Backend returned", data.length, "job(s). BaseURL:", baseURL, "Sample ids:", data.slice(0, 3).map((j: any) => j?.id));
        return data.map((j: any) => ({
          ...j,
          id: j.id != null ? String(j.id) : j.id,
          location: typeof j.location === "object" && j.location ? j.location : { lat: 13.736717, lng: 100.523186 },
          datetime: j.datetime || j.created_at,
          status: j.status || "open",
        }));
      } catch (backendError) {
        console.warn("[getJobs] Backend failed, falling back to Firebase. Error:", backendError);
      }
      ...
      console.log("[getJobs] Using Firebase fallback:", sorted.length, "job(s) (งานที่ Bob โพสต์ผ่าน backend จะไม่โผล่ที่นี่)");
      return sorted;
    } catch (e) {
  ...
  getYourJobs: async (userId?: string): Promise<Job[]> => {
    ...
    const normalizeJob = (j: any): Job => {
      ...
      if (!location || typeof location !== "object") location = { lat: 0, lng: 0 };
      return {
        ...j,
        id: j.id != null ? String(j.id) : j.id,
        created_by: j.created_by != null ? String(j.created_by) : j.created_by,
        accepted_by: j.accepted_by != null ? String(j.accepted_by) : j.accepted_by,
    ...
    try {
      try {
        const response = await api.get(`/users/jobs/${uid}`);
        const data = response.data || [];
        const list = (Array.isArray(data) ? data : []).filter((j: any) => j && j.id !== "mock-001" && j.id != null);
        if (list.length > 0) {
          console.log(`📦 Backend returned ${list.length} jobs for user ${uid}`);
        } else {
          console.warn(`📦 Backend returned 0 jobs for user ${uid} (ตรวจ backend: GET /api/users/jobs/${uid} ควรคืนงานที่ created_by/accepted_by = ${uid})`);
        }
  ...
  acceptJob: async (jobId: string): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        if (status === 404) {
          throw new Error(msg || "Job not found. It may have been created on the server—please refresh and try again.");
        }
        if (status === 400) {
          throw new Error(msg || "Job is not available.");
        }
        console.warn("Backend accept failed, trying Firestore fallback:", backendError?.message);
      }

      // Fallback to Firebase เฉพาะเมื่องานมีใน Firestore
      const jobRef = doc(db, "jobs", jobId);
      const jobSnap = await getDoc(jobRef);
      if (!jobSnap.exists()) {
        throw new Error("Job not found. This job was created on the server—please refresh the list and try again.");
      }
  ...
  cancelJob: async (jobId: string, reason?: string): Promise<void> => {
    ...
    try {
      await api.post(`/jobs/${jobId}/cancel`, { userId, reason: reason || "Cancelled by employer" });
      return;
    } catch (backendErr: any) {
      if (backendErr?.response?.status === 404 || backendErr?.response?.status === 403) {
        throw new Error(backendErr?.response?.data?.error || backendErr.message);
      }
      if (backendErr?.response?.status === 400) {
        throw new Error(backendErr?.response?.data?.error || "Cannot cancel job in current status");
      }
    }
    ...
    const job = mapDoc<Job>(jobSnap);
    if (job.created_by !== userId) throw new Error("Only job owner can cancel");
  ...
  requestJobCompletionOtp: async (jobId: string): Promise<{ success: boolean; message?: string }> => {
    try {
  ...
  subscribeToRecommendedJobs: (
    ...
    // โหลดจาก backend ทันที (รวมงานที่ Bob โพสต์) และ refresh ทุก 15 วินาที
    const refresh = () => MockApi.getRecommendedJobs().then(callback).catch(console.error);
    refresh();
  ...
  // ============================================
  // ✅ ADMIN BROADCAST NOTIFICATIONS (แสดงที่หน้า Home)
  // ============================================
  getLatestAdminNotifications: async (limit = 5): Promise<
    { id: string; title: string; message: string; target: string; sentAt: string }[]
  > => {
    const tryUrl = async (baseUrl: string) => {
      const url = `${baseUrl.replace(/\/$/, "")}/api/notifications/latest`;
      const res = await axios.get(url, { params: { limit }, timeout: 6000 });
      return res.data?.notifications ?? [];
    };
    let lastErr: any = null;
    try {
      const list = await tryUrl(BACKEND_URL);
    ...
    if (typeof window !== "undefined" && window.location?.hostname === "localhost") {
      try {
        return await tryUrl("http://localhost:3001");
      } catch (e2: any) {
        lastErr = e2;
      }
    }
  ...
  getRecommendedJobs: async (): Promise<Job[]> => {
    ...
    try {
      if (userId) {
        const response = await api.get("/jobs/recommended", { params: { userId } });
        const data = Array.isArray(response.data) ? response.data : response.data?.data || [];
        ...
        console.log("[getRecommendedJobs] Backend returned", jobs.length, "job(s) for user", userId);
        return jobs.slice(0, 50);
      }
    } catch (backendErr) {
      console.warn("[getRecommendedJobs] Backend failed, falling back to Firestore:", backendErr);
    }
    ...
    try {
      ...
      jobs = jobs.filter((j) => {
        const isOpen = j.status === JobStatus.OPEN || j.status?.toLowerCase() === "open";
        const notMyJob = j.created_by !== userId;
        const notAcceptedByMe = !j.accepted_by || j.accepted_by !== userId;
        const now = Date.now();
        const SEVEN_DAYS = 7 * 24 * 60 * 60 * 1000;
        const created = new Date(j.created_at || j.datetime).getTime();
        return isOpen && notMyJob && notAcceptedByMe && now - created < SEVEN_DAYS;
      });
      if (userSkills.length > 0) {
        const matching = jobs.filter((j) => userSkills.some((s) => (j.category || "").toLowerCase().includes((s || "").toLowerCase())));
        const other = jobs.filter((j) => !matching.includes(j));
        jobs = [...matching, ...other];
      }
      jobs.sort((a, b) => new Date(b.datetime).getTime() - new Date(a.datetime).getTime());
      return jobs.slice(0, 10);
    } catch (e) {
      console.warn("Failed to fetch recommended jobs:", e);
      return MOCK_JOBS.filter((job) => job.status === JobStatus.OPEN)
        .slice(0, 3)
        .map((job) => ({ ...job, distance: Math.floor(Math.random() * 10) + 1 }));
    }
  },
  ...
  createSupportTicket: async (params: {
    ...
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const res = await axios.post(`${base}/api/support/tickets`, params, { timeout: 8000 });
    return res.data;
  },
  createDisputeSupportTicket: async (jobId: string, userId: string, reason: string): Promise<{ ticket: { id: string } }> => {
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const res = await axios.post(`${base}/api/support/tickets/from-dispute`, { jobId, userId, reason }, { timeout: 8000 });
    return res.data;
  },
  getMySupportTickets: async (userId: string): Promise<{ tickets: Array<{ id: string; subject: string; status: string; lastUpdated: string; category: string; source?: string; jobId?: string }> }> => {
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const res = await axios.get(`${base}/api/support/tickets`, { params: { userId, limit: 20 }, timeout: 8000 });
    return res.data;
  },
  getSupportTicketMessages: async (ticketId: string): Promise<{ messages: Array<{ id: string; sender: string; message: string; timestamp: string }> }> => {
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const res = await axios.get(`${base}/api/support/tickets/${ticketId}/messages`, { timeout: 8000 });
    return res.data;
  },
  sendSupportMessage: async (ticketId: string, message: string): Promise<{ message: { id: string } }> => {
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const res = await axios.post(`${base}/api/support/tickets/${ticketId}/messages`, { message }, { timeout: 8000 });
    return res.data;
  },

  // แบนเนอร์หน้า Home (จาก Content Manager) + โค้ดส่วนลด
  getBanners: async (): Promise<{ banners: Array<{ id: string; title: string; imageUrl: string; actionUrl?: string; order: number; startDate?: string; endDate?: string; isActive: boolean; promoCode?: string; discountMaxBaht?: number; discountDescription?: string }> }> => {
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const res = await axios.get(`${base}/api/banners`, { timeout: 8000 });
    return res.data;
  },
  claimVoucher: async (code: string): Promise<{ voucher: { id: string; promoCode: string; remainingBaht: number }; message: string }> => {
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const userId = localStorage.getItem("meerak_user_id");
    if (!userId) throw new Error("กรุณาเข้าสู่ระบบก่อนรับโค้ด");
    const res = await axios.post(`${base}/api/vouchers/claim`, { code, userId }, { timeout: 8000 });
    return res.data;
  },
  getMyVouchers: async (): Promise<{ vouchers: Array<{ id: string; promoCode: string; remainingBaht: number; maxDiscountBaht: number; expiresAt?: string }> }> => {
    const base = typeof window !== "undefined" && window.location?.hostname === "localhost" ? "http://localhost:3001" : BACKEND_URL;
    const userId = localStorage.getItem("meerak_user_id");
    if (!userId) return { vouchers: [] };
    const res = await axios.get(`${base}/api/vouchers/my`, { params: { userId }, timeout: 8000 });
    return res.data;
  },
  ...
  acceptJobAsProvider: async (
    ...
    try {
      try {
        const res = await api.post(`/jobs/${jobId}/accept`, { userId: providerId });
        const job = res.data?.job;
        if (job) return { ...job, id: String(job.id), status: job.status || "accepted" } as Job;
      } catch (be: any) {
        if (be?.response?.status === 404) throw new Error(be?.response?.data?.error || "Job not found");
        if (be?.response?.status === 400) throw new Error(be?.response?.data?.error || "Job is not available");
      }
      ...
      const job = mapDoc<Job>(jobSnap);
      if (job.status !== JobStatus.OPEN) throw new Error("Job is not available");
  ...
  getProviderOnboardingStatus: async (userId: string): Promise<{
    provider_status: string;
    provider_verified_at: string | null;
    provider_test_next_retry_at: string | null;
    provider_test_attempts: number;
    onboarding_status?: string;
    exam_results?: { module: number; category?: string | null; attempt: number; score: number; passed: boolean; submitted_at: string | null; time_spent_seconds?: number | null }[];
  }> => {
    try {
      const res = await api.get("/provider-onboarding/status", { params: { userId } });
      return res.data;
    } catch (e) {
      return {

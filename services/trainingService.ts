 ...
 */
import { Course, CourseCategory, Lesson, Progress, Quiz } from '../types';
 ...
 async submitQuiz(userId: string, courseId: string, lessonId: string, answers: Record<string, any>): Promise<{ score: number; passed: boolean; attempts: number; nextRetryAt?: string }> {
    try {
      if (courseId === NEXUS_PROFESSIONAL_COURSE_ID) {
        try {
          const res = await api.post<{ passed: boolean; score: number; nextRetryAt?: string }>('/provider-onboarding/submit-exam', { userId, answers });
          const data = res.data;
          if (data.passed) {
            await this.markCompleted(userId, courseId, lessonId);
            return { score: data.score, passed: true, attempts: 1 };
          }
          return { score: data.score, passed: false, attempts: 1, nextRetryAt: data.nextRetryAt };
        } catch (err: any) {

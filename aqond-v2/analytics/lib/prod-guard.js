...
export function assertProdSecrets(checks) {
  ...
  console.error("[FATAL] AQOND_ENV=production — rotate weak/missing secrets:", bad.map((c) => c.name).join(", "));
  process.exit(1);
}

export function requireApiKey(envValue, headerNames = ["x-api-key"]) {
  return (req, res, next) => {
    if (!isProduction() && !envValue) return next();
    if (!envValue) return res.status(503).json({ error: "api_key_not_configured" });
...
export function strictSyncSecret(webhookSecret) {
  return (req, res, next) => {
    ...
    if (webhookSecret && hdr !== webhookSecret) return res.status(403).json({ error: "invalid_secret" });
    if (isProduction() && !webhookSecret) return res.status(503).json({ error: "sync_not_configured" });
    return next();
  };
}

#!/usr/bin/env node
/** Receipt Core — B2.6-S003 signed verify QR */
...
async function main() {
  ...
  const unsigned = await fetch(
    `${BASE}/api/receipt/v1/verify?order_id=${ORDER.order_id}&buyer_id=${ORDER.buyer_id}`,
  );
  push('S003-1', 'Reject unsigned verify', unsigned.status === 403);

  const qs = new URLSearchParams({ order_id: ORDER.order_id, buyer_id: ORDER.buyer_id, v: signToken(ORDER.order_id) });
  const signed = await fetch(`${BASE}/api/receipt/v1/verify?${qs}`);
  const body = await signed.json().catch(() => ({}));
  push('S003-2', 'Accept signed token', signed.ok && body.verified === true);
  push('S003-3', 'Scenario tag', body.scenario === 'B2.6-S003');

  const pdf = await fetch(
    `${BASE}/api/orders/${ORDER.order_id}/receipt.pdf?buyer_id=${ORDER.buyer_id}`,
  );
  const verifyHeader = pdf.headers.get('x-aqond-receipt-verify') || '';
  push('S003-4', 'Receipt verify URL signed', verifyHeader.includes('v='));

  const status = checks.every((c) => c.pass) ? 'PASS' : 'FAIL';
  console.log(JSON.stringify({ scenario: 'B2.6-S003', mission: 'RECEIPT-CORE', status, checks }, null, 2));

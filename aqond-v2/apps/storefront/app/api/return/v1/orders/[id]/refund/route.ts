...
import { getRefundDetailForOrder } from '@/lib/server/returnService';

export const dynamic = 'force-dynamic';

type Params = { params: { id: string } };

/** B2.7-S002 — OR002 Refund detail by order id. */
export async function GET(req: NextRequest, { params }: Params) {
  const buyerId = req.nextUrl.searchParams.get('buyer_id') || '';
  try {
    const detail = await getRefundDetailForOrder(params.id, buyerId || undefined);
    if (!detail) {

...
/** Adapter for user.controller - wraps wallet functions */
export class WalletService {
  constructor(_pool: unknown) {}
  async getWalletSummary(userId: string) {
    return getBalance(userId);
  }
  async deposit(_userId: string, _amount: number, _description?: string, _metadata?: unknown) {
    throw new Error("Use topup() for deposits - not implemented via controller");
  }
  async withdraw(_userId: string, _amount: number, _description?: string, _metadata?: unknown) {
    throw new Error("Use withdraw() with idempotency - not implemented via controller");
  }
  async getTransactionHistory(_userId: string, _limit: number, _offset: number) {
    return [];
  }
}

...
declare global {
  namespace Express {
    interface Request {
      user?: { id: string; role: AppRole; email?: string; firebase_uid?: string };
    }
  }
}
...
export function authenticate(
  ...
  try {
    ...
    const role: AppRole = ["USER", "ADMIN", "AUDITOR"].includes(
      ...
      : "USER";
    req.user = { id: sub, role, email: payload.email, firebase_uid: payload.firebase_uid ?? sub };
    next();
  } catch (e) {
...
export function optionalAuthenticate(
  ...
  try {
    ...
    const role: AppRole = ["USER", "ADMIN", "AUDITOR"].includes(
      ...
      ? (payload.role as AppRole)
      : "USER";
    req.user = { id: sub, role, email: payload.email, firebase_uid: payload.firebase_uid };
    next();
  } catch {
    next();
  }

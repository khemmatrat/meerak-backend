...
class WebhookController {
  /**
   * Webhook สำหรับช่องทาง Thai QR / e-wallet (รูปแบบ payload ตามผู้ให้บริการที่ต่อภายหลัง)
   */
  async handleThaiQrPaymentWebhook(req: Request, res: Response): Promise<void> {
    try {
      const signature =
      ...
      if (!isValid) {
        console.error("Invalid Thai QR / wallet webhook signature");
      ...
      console.log("Thai QR / wallet webhook event:", event.key);
      ...
      switch (event.key) {
        ...
        default:
          console.log("Unhandled Thai QR / wallet webhook event:", event.key);
      }
      ...
      console.error("Thai QR / wallet webhook error:", error);

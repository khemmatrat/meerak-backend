/**
 * เทมเพลตสำหรับคัดลอกทับ `mobile/vite.config.ts` (ถ้าเครื่อง/agent เขียนโฟลเดอร์ mobile แล้ว EPERM).
 * Copy-Item .\backend\scripts\mobile-vite.config.attrib-patch.ts .\mobile\vite.config.ts -Force
 */
...
/** Windows: Read-only ทำให้ copyFileSync ได้ EPERM — เทียบเท่า attrib -R ต่อไฟล์ (ราก + public) */
function stripReadOnlyIfWin32(absPath: string) {
  if (process.platform !== 'win32' || !existsSync(absPath)) return;
  try {
    execFileSync('attrib', ['-R', absPath], { stdio: 'ignore' });
  } catch {
    /* best-effort */
  }
}
  ...
  return {
    ...
    buildStart() {
      ...
      for (const file of THEME_CSS_LINKED_FROM_HTML) {
        ...
        try {
          if (existsSync(src)) {
            stripReadOnlyIfWin32(src);
            stripReadOnlyIfWin32(dest);

...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [adminNotifications, setAdminNotifications] = useState<{ id: string; title: string; message: string; sentAt: string }[]>([]);
  ...
  useEffect(() => {
      const fetchData = async () => {
          if (user) {
              const [notifs, adminList] = await Promise.all([
                  MockApi.getNotifications(),
                  MockApi.getLatestAdminNotifications(5).catch(() => []),
              ]);
  ...
  const mergedNotifications: UserNotification[] = [
                              ...
                              {mergedNotifications.length === 0 ? (
                                  ...
                                  mergedNotifications.map(n => (
                                      <div 
                                        ...
                                        className={`p-3 border-b border-gray-50 cursor-pointer hover:bg-gray-50 transition-colors ${!n.is_read ? 'bg-emerald-50/50' : ''} ${n.type === 'admin_broadcast' ? 'bg-indigo-50/50' : ''}`}
                                      >
                                          <div className="flex items-start gap-3">
                                              <div className={`mt-1 p-1.5 rounded-full flex-shrink-0 ${n.type === 'admin_broadcast' ? 'bg-indigo-100 text-indigo-600' : !n.is_read ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}>
                                                  {n.type === 'admin_broadcast' ? <Bell size={14} /> : n.type === 'job_match' ? <Sparkles size={14} /> : <Info size={14} />}
                                              </div>
                                              <div className="min-w-0 flex-1">
                                                  <div className="flex items-center gap-2 flex-wrap">
                                                      ...
                                                      {n.type === 'admin_broadcast' && (
                                                          <span className="text-[10px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded">จากแอดมิน</span>
                                                      )}
                                                  </div>
                                                  <p className="text-xs text-gray-500 line-clamp-2 mt-0.5">{n.message}</p>
                                                  <p className="text-[10px] text-gray-400 mt-1">{new Date(n.created_at).toLocaleString('th-TH')}</p>
                                              </div>
                                          </div>
                                      </div>
                                  ))
                   ...
                   <div className="absolute top-16 right-2 left-2 bg-white rounded-xl shadow-xl border border-gray-200 z-50 max-h-[60vh] overflow-y-auto">
                       ...
                       {mergedNotifications.length === 0 ? (
                            ...
                            mergedNotifications.map(n => (
                                ...
                                key={n.id} 
                                onClick={() => handleNotifClick(n)}
                                className={`p-3 border-b border-gray-50 cursor-pointer ${!n.is_read ? 'bg-emerald-50/50' : ''} ${n.type === 'admin_broadcast' ? 'bg-indigo-50/50' : ''}`}
                                >
                                    <div className="flex items-center gap-2">
                                        <p className={`text-sm ${!n.is_read ? 'font-bold' : ''}`}>{n.title}</p>
                                        {n.type === 'admin_broadcast' && <span className="text-[10px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded">จากแอดมิน</span>}
                                    </div>

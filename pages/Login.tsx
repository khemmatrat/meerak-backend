...
export const Login: React.FC = () => {
  ...
  // Countdown for rate limit (429) — Smart Retry
  useEffect(() => {
    if (rateLimitRetryAfter > 0) {
      const t = setTimeout(() => setRateLimitRetryAfter((n) => Math.max(0, n - 1)), 1000);
      return () => clearTimeout(t);
    }
  }, [rateLimitRetryAfter]);

  const demoPassword = (typeof import.meta !== "undefined" && (import.meta as any).env?.VITE_DEMO_PASSWORD) || (typeof process !== "undefined" && process.env?.REACT_APP_DEMO_PASSWORD) || "";

  // Old login (for demo buttons — ใช้รหัสจาก .env เท่านั้น ไม่ hardcode)
  const handleLogin = async (e?: React.FormEvent, demoPhone?: string) => {
    if (e) e.preventDefault();
    if (demoPhone && !demoPassword) {
      setError("Demo login is disabled. Set VITE_DEMO_PASSWORD or REACT_APP_DEMO_PASSWORD in .env to enable.");
      return;
    }
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const pass = demoPhone ? demoPassword : password;
      const { token, user } = await MockApi.login(p, pass);
      login(user, token);
      handleLoginSuccess(user);
    } catch (error: any) {
      setError(error.message || "Login failed");
      const retry = typeof (error as any).retry_after === "number" ? (error as any).retry_after : 0;
      if (retry > 0) setRateLimitRetryAfter(retry);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        const displayMsg =
          (msg.includes("Firebase") || msg.includes("password login")
            ? 'OTP is not available. Please use "Sign in with password" below.'
            : msg) || "Failed to send OTP";
        setError(displayMsg);
        return;
      }

      setOtpId(result.id ?? null);
      setStep("otp");
      ...
      const retry = error?.retry_after ?? error?.response?.data?.retry_after;
      if (typeof retry === "number" && retry > 0) setRateLimitRetryAfter(retry);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
        ...
        {error && (
          <div className="mb-6 space-y-3">
            ...
            {(rateLimitRetryAfter > 0 || /too many|rate limit|429/i.test(error)) && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm">
                <p className="font-medium text-amber-800 mb-2">
                  ลองใหม่อีกครั้งใน <span className="font-mono font-bold">{rateLimitRetryAfter}</span> วินาที
                </p>
        ...
        {/* Demo Quick Login — แสดงเฉพาะเมื่อตั้ง VITE_DEMO_PASSWORD / REACT_APP_DEMO_PASSWORD ใน .env (ไม่ยัดรหัสในโค้ด) */}
        {(typeof import.meta !== "undefined" && (import.meta as any).env?.VITE_DEMO_PASSWORD) || (typeof process !== "undefined" && process.env?.REACT_APP_DEMO_PASSWORD) ? (
        ...
        {/* Step 1: Enter Phone Number + OTP or Password */}
        {step === "phone" && (
          <>
            ...
            <form onSubmit={(e) => handleLogin(e)} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 px-4 bg-slate-700 hover:bg-slate-800 text-white font-bold rounded-lg transition-colors disabled:opacity-50"
              >
        ...
        <div className="mt-8 pt-6 border-t border-gray-100 text-center">
          <span className="inline-flex items-center text-xs text-slate-400">
            <Shield size={12} className="mr-1" /> Admin Portal: nexus-admin-core
          </span>
        </div>

...
useEffect(() => {
  ...
  const handleSystemAutoApprove = async () => {
    ...
    try {
      ...
      // 2. โอนเงิน (processPayment return UserProfile)
  ...
  const handleAcceptJob = async () => {
    ...
    try {
      // 💰 Phase 5: กันเงินจาก Employer เมื่อ Provider Accept งาน
      console.log('💰 Holding payment for job:', id);
      ...
      if (!holdSuccess) {
        notify("❌ ไม่สามารถกันเงินได้", "error");
        return;
      }
      ...
      console.log('✅ Payment held and job accepted');
    } catch (err: any) {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            // ส่งงาน
            await MockApi.markJobAsDone(id, currentLoc);
  ...
  const handleApproveWork = async () => {
    ...
    const confirmMsg = `ยืนยันการอนุมัติงานและโอนเงิน ${job.price} บาทให้ผู้รับงาน?`;
  ...
  const handleRequestWithdrawal = async () => {
    ...
    const confirmMsg = `ยืนยันการขอถอนเงิน ${job.price} บาท?`;
  ...
  const canAutoPay = (user?.wallet_balance || 0) >= job.price;
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-green-700 mt-1">
                        จำนวน: <span className="font-bold text-2xl">{job.price.toLocaleString()} บาท</span>
                      </p>
                    </div>

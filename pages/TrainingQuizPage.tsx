    ...
    try {
      ...
      if (res.passed) {
        ...
        if (course.id === 'nexus-professional-standards') {
          alert(`ผ่าน Module 1 🎉 คะแนน ${res.score}%\nกรุณาทำ Module 2 (ทักษะทางเทคนิค) ตามอาชีพ แล้วทำ Module 3 (ทัศนคติ)\nเมื่อผ่าน M1+M2 แล้ว แอดมินจะตรวจและกดอนุมัติให้รับงานได้`);
        ...
        const msg = nextRetry
          ? `ไม่ผ่าน คะแนน ${res.score}%. กรุณารอ 24 ชั่วโมงก่อนทำใหม่ (ทำได้หลัง ${new Date(nextRetry).toLocaleString('th-TH')})`
          : `ไม่ผ่าน คะแนน ${res.score}%. กรุณากลับไปดูวิดีโอแล้วลองใหม่`;
        alert(msg);
      ...
      if (err?.nextRetryAt) {
        const msg = `${err.message}\nทำได้หลัง ${new Date(err.nextRetryAt).toLocaleString('th-TH')}`;
        alert(msg);
        navigate(`/training/course/${course.id}`);
